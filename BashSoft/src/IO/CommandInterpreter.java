...
public class CommandInterpreter {
    ...
    public static void interpretCommand(String input) {
        ...
        switch (command){
            case "open": tryOpenFile(input, data);
                break;

            case "mkdir": tryCreateDirectory(input, data);
                break;

            case "ls": tryTraverseFolders(input, data);
                break;

            case "cmp": tryCompareFiles(input, data);
                break;

            case "changeDirRel": tryChangeRelativePath(input, data);
                break;

            case "changeDirAbs": tryChangeAbsolutePath(input, data);
                break;

            case "readDb": tryReadDatabaseFromFile(input, data);
                break;

            case "filter": tryPrintFilteredStudents(input, data);
                break;

            case "order": tryPrintOrderedStudents(input, data);
                break;

            case "download": tryDownloadFile(input, data);
                break;

            case "downloadAsynch": tryDownloadFileOnNewThread(input, data);
                break;

            case "show": tryShowWantedCourse(input, data);
                break;
    ...
    private static void tryPrintOrderedStudents(String input, String[] data) {
        ...
        if(data.length == 3){
            StudentRepository.printOrderedStudents(course, comparisonType, null);
            return;
        }
    ...
    private static void tryReadDatabaseFromFile(String input, String[] data) {
        ...
        try {
            ...
            OutputWriter.writeMessageOnNewLine("Error with reading file.");
        }
    }
    ...
    private static void tryOpenFile(String input, String[] data) {
        ...
        try {
            ...
            OutputWriter.writeMessageOnNewLine("Error with opening the new file.");
        }
    }
